...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        // 倒计时策略：记录每个市场是否已经下过单
        let countdown_strategy_ordered: Arc<DashMap<B256, bool>> = Arc::new(DashMap::new());
                    ...
                    match book_result {
                        Some(Ok(book)) => {
                            ...
                            if let Some(pair) = monitor.handle_book_update(book) {
                                ...
                                // 计算5分钟倒计时
                                let countdown_info = market_info
                                    .map(|info| {
                                        use chrono::Utc;
                                        let now = Utc::now();
                                        let time_until_end = info.end_date.signed_duration_since(now);
                                        let seconds_until_end = time_until_end.num_seconds();
                                        if seconds_until_end > 0 {
                                            format!("倒计时:{:02}:{:02}", seconds_until_end / 60, seconds_until_end % 60)
                                        } else {
                                            "已结束".to_string()
                                        }
                                    })
                                ...
                                // 倒计时策略：每一局倒计时1分钟到50秒的时候，yes或者no下单较高价格的，只下一单
                                if config.enable_countdown_strategy {
                                    // 检查该市场是否已经下过单
                                    if !countdown_strategy_ordered.contains_key(&pair.market_id) {
                                        // 获取市场信息
                                        if let Some(market_info) = market_map.get(&pair.market_id) {
                                            use chrono::Utc;
                                            let now = Utc::now();
                                            ...
                                            // 检查是否在倒计时策略的时间范围内（1分钟到50秒）
                                            if seconds_until_end >= config.countdown_strategy_end_secs as i64 && 
                                               seconds_until_end <= config.countdown_strategy_start_secs as i64 {
                                                // 获取YES和NO的卖一价
                                                let yes_best_ask = pair.yes_book.asks.last();
                                                let no_best_ask = pair.no_book.asks.last();
                                                
                                                if let (Some(yes_ask), Some(no_ask)) = (yes_best_ask, no_best_ask) {
                                                    // 比较价格，选择较高的价格下单
                                                    let (token_id, price, size, side) = if yes_ask.price >= no_ask.price {
                                                        (pair.yes_book.asset_id, yes_ask.price, yes_ask.size, "YES")
                                                    } else {
                                                        (pair.no_book.asset_id, no_ask.price, no_ask.size, "NO")
                                                    };
                                                    
                                                    // 计算订单大小为1除以对应的单价，确保不超过可用大小
                                                    let order_size = size.min(dec!(1.0) / price);
                                                    
                                                    // 检查订单大小是否大于0
                                                    if order_size > dec!(0) {
                                                        // 标记该市场已经下过单
                                                        countdown_strategy_ordered.insert(pair.market_id, true);
                                                        
                                                        info!(
                                                            "⏰ 执行倒计时策略 | 市场:{} | 距离结束:{}秒 | 下单:{} | 价格:{:.4} | 数量:{}份",
                                                            market_display,
                                                            seconds_until_end,
                                                            side,
                                                            price,
                                                        ...
                                                        let order_size_clone = order_size;
                                                        let market_id_clone = pair.market_id;
                                                        
                                                        // 使用 tokio::spawn 异步执行下单，不阻塞订单簿更新处理
                                                        tokio::spawn(async move {
                                                            // 执行限价买单
                                                            use alloy::signers::local::LocalSigner;
                                                            ...
                                                            // 获取执行器的私有字段
                                                            let private_key = executor_clone.private_key.clone();
                                                            let client = &executor_clone.client;
