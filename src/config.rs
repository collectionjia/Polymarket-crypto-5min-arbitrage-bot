...
pub struct Config {
    ...
    /// 倒计时策略开始时间（距离市场结束的秒数），默认60秒（1分钟）
    pub countdown_strategy_start_secs: u64,
    /// 倒计时策略结束时间（距离市场结束的秒数），默认50秒
    pub countdown_strategy_end_secs: u64,
    /// 倒计时策略的最大订单大小（USDC），默认100.0
    pub countdown_strategy_max_order_size_usdc: f64,
}
    ...
    pub fn from_env() -> Result<Self> {
        ...
        Ok(Config {
            ...
            enable_countdown_strategy: env::var("ENABLE_COUNTDOWN_STRATEGY")
                ...
                .parse()
                .unwrap_or(false), // 默认false
            countdown_strategy_start_secs: env::var("COUNTDOWN_STRATEGY_START_SECS")
                .unwrap_or_else(|_| "60".to_string())
                .parse()
                .unwrap_or(60), // 默认60秒
            countdown_strategy_end_secs: env::var("COUNTDOWN_STRATEGY_END_SECS")
                .unwrap_or_else(|_| "50".to_string())
                .parse()
                .unwrap_or(50), // 默认50秒
            countdown_strategy_max_order_size_usdc: env::var("COUNTDOWN_STRATEGY_MAX_ORDER_SIZE_USDC")
                .unwrap_or_else(|_| "100.0".to_string())
                .parse()
                .unwrap_or(100.0), // 默认100.0
        })
    }
}
